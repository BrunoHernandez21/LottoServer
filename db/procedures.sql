...
##-- Determinando 
DROP PROCEDURE IF EXISTS `pago_unico_ADD`;
DROP PROCEDURE IF EXISTS `pago_unico`;
delimiter $$
CREATE PROCEDURE pago_unico(IN iorden_id int, IN stripe_key varchar(60)) 
begin
    ...
    SET pcash = (SELECT puntos_total FROM ordenes WHERE i.orden_id = iorden_id);
...
DROP PROCEDURE IF EXISTS `suscribcion`;
delimiter $$
CREATE PROCEDURE suscribcion(IN iorden_id int, IN stripe_key varchar(60)) 
begin
    DECLARE pcash bigint;
    DECLARE ccash bigint;
    DECLARE user_id bigint;
